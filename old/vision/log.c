...
int InitLogs(char *mode){
	...
	if(LOGGING){
		...
		//printf("%s\n\n",log_fname);
		snprintf(cam_log_fname,sizeof(cam_log_fname),"cam_%s%s",mode,log_fname);
		snprintf(time_log_fname,sizeof(time_log_fname),"time_%s%s",mode,log_fname);
		//printf("%s\n\n",cam_log_fname);
		//printf("%s\n\n",time_log_fname);
