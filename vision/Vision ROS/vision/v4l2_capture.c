...
int InitCamera(){
	...
	int brightness, exposure_auto, exposure_absolute, contrast, saturation,gamma,hue;

	FILE *ff;
    	ff=fopen(WEBCAM_FILE,"r");
    	if(ff==NULL) 
		printf("Error on file opening\n");
	fscanf(ff,"%d\n%d\n%d\n%d\n%d\n%d\n%d\n", &brightness, &exposure_auto, &exposure_absolute, &contrast, &saturation,&gamma,&hue);
	//printf("\n\n\n%d\t%d\t%d\t%d\t%d\n\n\n",brightness, exposure_auto, exposure_absolute, contrast,saturation);
	...
	if (-1 == ioctl (fg_v4l2->fd, VIDIOC_QUERYCTRL, &queryctrl)) {
        	if (errno != EINVAL) {
	...
	memset (&control, 0, sizeof (control));
        ...
        control.value =brightness; //queryctrl.minimum;


       if (-1 == ioctl (fg_v4l2->fd, VIDIOC_S_CTRL, &control)) {
	...
	if (-1 == ioctl (fg_v4l2->fd, VIDIOC_QUERYCTRL, &queryctrl)) {
        	...
        	printf ("V4L2_CID_EXPOSURE_ABSOLUTE  is not supported\n");
		        ...
		        control.value = exposure_absolute;
